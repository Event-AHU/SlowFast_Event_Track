...
class EventVOT(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        ...
        bin_event_path = '{}/{}/{}'.format(self.base_path, sequence_name, sequence_name+'_bin')
        bin_event_list = [frame for frame in os.listdir(bin_event_path) if frame.endswith(".bin")]
        bin_event_list.sort(key=lambda f: int(f[-8:-4]))
        bin_event_list = [os.path.join(bin_event_path, frame) for frame in bin_event_list]

        return Sequence(sequence_name, frames_list, 'EventVOT', ground_truth_rect.reshape(-1, 4), bin_event_list=bin_event_list)
