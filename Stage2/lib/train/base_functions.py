...
# datasets related
from lib.train.dataset import EventVOT, Coesot, FE240
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["COESOT", "COESOT_VAL", "FE240", "FE240_VAL", "EventVOT", "EventVOT_VAL"]
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
                {
                    "params": [p for n, p in net.named_parameters() if "fast_vit" in n and p.requires_grad],
                    "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
                },
                {
                    "params": [p for n, p in net.named_parameters() if "fast_gcn" in n and p.requires_grad],
                    "lr": cfg.TRAIN.LR_GCN,
                },
                {
                    "params": [p for n, p in net.named_parameters() if "fast_head" in n and p.requires_grad],
                    
                },
            ]
        
        # param_dicts = [{"params": [p for n, p in net.named_parameters() if "fast_head" in n and p.requires_grad]}]
            
    if cfg.TRAIN.OPTIMIZER == "ADAMW":
        optimizer = torch.optim.AdamW(param_dicts, lr=cfg.TRAIN.LR,
                                      weight_decay=cfg.TRAIN.WEIGHT_DECAY)
        print("Learnable parameters and their learning rates:")
