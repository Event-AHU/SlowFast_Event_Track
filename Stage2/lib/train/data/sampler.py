...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
                            allow_invisible=False, force_invisible=False, is_search=False):
        ...
        if is_search:
            if len(valid_ids) < num_ids+1:
                return None
            new_valid_ids = random.randint(0, len(valid_ids) - num_ids)
            return valid_ids[new_valid_ids:new_valid_ids + num_ids]
    ...
    def getitem(self):
        ...
        while not valid:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        ...
                        search_frame_ids = self._sample_visible_ids(visible, min_id=template_frame_ids[0] + 1,
                                                                  ...
                                                                  num_ids=self.num_search_frames, is_search=True)
            ...
            try:
                template_frames, template_anno, meta_obj_train, template_bin_event = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict, get_bin=True)
                search_frames, search_anno, meta_obj_test, search_bin_event = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict, get_bin=True)
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'template_bin_event': template_bin_event,
                                   'search_bin_event': search_bin_event,
                                   'dataset': dataset.get_name(),
                                   'test_class': meta_obj_test.get('object_class_name')})
                # data = self.build_graph(data, H, W)
