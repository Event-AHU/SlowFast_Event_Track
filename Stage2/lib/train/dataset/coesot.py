...
class Coesot(BaseVideoDataset):
    ...
    def _get_event_sequence_path(self, seq_id):        ## get event bin
        return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_bin")
    
    def _get_event_bin_sequence_path(self, seq_event_bin_path, frame_ids):
        bin_path = os.path.join(seq_event_bin_path, 'frame{:04}.bin'.format(frame_ids[-1]))
    ...
    def get_frames(self, seq_id, frame_ids, anno=None, get_bin=False):
        ...
        object_meta = OrderedDict({'object_class_name': None,
                                   ...
                                   'motion_adverb': None})
        if get_bin:
            seq_event_bin_path = self._get_event_sequence_path(seq_id)
            bin_event_data = self._get_event_bin_sequence_path(seq_event_bin_path, frame_ids)
            
            return frame_event_img_list, anno_frames, object_meta, bin_event_data
        else:
            return frame_event_img_list, anno_frames, object_meta, None
